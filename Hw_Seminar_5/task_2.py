...
names = input('Укажите имена через пробел: ').split()
salary = input('Укажите ставку через пробел: ').split()
bonus = input('Укажите процент премий через пробел со знаком процента (10.25%) : ').split()

my_dict = {names: float(salary) * (float(bonus[:-1]) / 100) for names, salary, bonus in zip(names, salary, bonus)}
